    ...
    for (auto & p : particles) {
        ...
        if (!p_landmarks.empty()) {
            ...
            for(const auto & landmark : p_landmarks) {
                ...
                // update weight with the gaussian probability.
                double zx = (x - landmark.x) / std_landmark[0];
                double zy = (y - landmark.y) / std_landmark[1];
                double prob = 0.5 * M_1_PI * exp(-0.5 * (zx * zx + zy * zy)) / (std_landmark[0] * std_landmark[1]);
                weight *= prob;
            }
            // Update the particle weight
            p.weight = weight;
        } else {
            // If no landmarks in range ignore all observations.
            p.weight = 0.0;
        }
    }
    
    // Copy all the particle weights.
